...
filename <- paste('weight-CF_wtCovariate_noInter_n', n, '.RData', sep='')
save.image(filename)
...
stat <- foreach (i=1:nsim, .combine=rbind) %dopar% {
  ...
  D.pro <- resid(lm(D.A1 ~ W.A1 + delta.hat))
  se <- sqrt(diag(solve(crossprod(D.pro)) * sum(eps.hat^2)/cf.result$df.residual))[1:P]
  ...
  deno <- crossprod(temp)
  rm(X, Z, delta, eps, D, H, Y, cf.result, Y.A1, H.pro, temp)
  gc()
...
filename <- paste0('simulation-CF_wtCovariate_noInter_n', n, '.rds')
...
# Analysis -------------------------------------------------------------------
filepath <- paste0('simulation-CF_wtCovariate_noInter_n'
                  , c(1000, 3000, 5000)
                  , '.rds')

bias.mat = se.mat = cover.mat <- matrix(NA, 3, 2)
rownames(bias.mat) <- rownames(se.mat) <- rownames(cover.mat) <- paste0('n=', c(1000, 3000, 5000))
colnames(bias.mat) = c('Empirical bias', 'Estimated bias')
colnames(se.mat) = c('Calculated SE', 'Empirical SE')
colnames(cover.mat) = c('Original coverage', 'Corrected coverage')
for (i in 1:length(filepath)) {
  result <- readRDS(filepath[i])
  bias.mat[i, ] = colMeans(result)[3:4]
  se.mat[i, 1] = colMeans(result)[2]
  se.mat[i, 2] = sqrt(mean((result[, 1] - colMeans(result)[1])^2))
  cover.mat[i, ] = colMeans(result)[5:6]
}
t(bias.mat)
t(se.mat)
t(cover.mat)
